    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-4 sm:py-8 space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold text-[#292824] tracking-tight flex items-center gap-2">
            ...
            {t('worker.notifications.title', 'Notifications')}
            {unreadCount > 0 && (
              <span className="px-2 py-0.5 bg-[#C93B2B] text-white text-xs font-bold rounded-full">
                {t('worker.notifications.newCount', { count: unreadCount, defaultValue: `${unreadCount} new` })}
              </span>
            )}
          </h1>
          <p className="text-xs text-[#77736B] mt-1">{t('worker.notifications.subtitle', 'Stay updated on jobs, payments, and announcements')}</p>
        </div>
        {unreadCount > 0 && (
          <button
            type="button"
          ...
          >
            ...
            {t('worker.notifications.markAllRead', 'Mark all read')}
          </button>
        )}
      </div>
      ...
      {storeWorkerNotifs.length === 0 && mockNotifs.length === 0 && (
        <div className="text-center py-16 space-y-3">
          ...
          <h3 className="text-base font-bold text-[#292824]">{t('worker.notifications.noNotifsTitle', 'No notifications yet')}</h3>
          <p className="text-xs text-[#77736B]">{t('worker.notifications.noNotifsSub', 'Job updates, payments, and announcements will appear here.')}</p>
        </div>
      )}
      ...
      {storeWorkerNotifs.length > 0 && (
        <div className="space-y-2">
          <h2 className="text-xs font-bold uppercase tracking-wider text-[#77736B]">{t('worker.notifications.systemNotifs', 'System Notifications')}</h2>
      ...
      {mockNotifs.length > 0 && (
        <div className="space-y-2">
          {storeWorkerNotifs.length > 0 && (
            <h2 className="text-xs font-bold uppercase tracking-wider text-[#77736B]">{t('worker.notifications.recentActivity', 'Recent Activity')}</h2>
          )}
          {mockNotifs.map((n) => {

...
export const DocumentReviewModal: React.FC<DocumentReviewModalProps> = ({
  ...
  const getStatusBadge = (status: DocumentStatus) => {
    switch (status) {
      case 'APPROVED':
        return <Badge variant="verified" size="sm">{t('admin.approvedBadge', '✓ Approved')}</Badge>;
      case 'REJECTED':
        return <Badge variant="danger" size="sm">{t('admin.rejectedBadge', '✗ Rejected')}</Badge>;
      case 'CORRECTION_REQUIRED':
        return <Badge variant="urgent" size="sm">{t('admin.needsCorrectionBadge', 'Needs Correction')}</Badge>;
      default:
        return <Badge variant="pending" size="sm">{t('admin.pendingReviewBadge', 'Pending Review')}</Badge>;
    }
  };
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={t('admin.documentReview', 'Verification Document Review')}
      ...
      <div className="space-y-5">
        ...
        <div className="p-5 bg-gradient-to-br from-[#FAF7F2] to-[#F3EEE4] border-2 border-[#E8E2D5] rounded-2xl shadow-subtle relative overflow-hidden">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <span className="text-[10px] font-black uppercase tracking-widest text-[#77736B] block leading-none">
                  {t('admin.officialRecord', 'Official Verification Record')}
                </span>
                <span className="text-xs font-bold text-[#292824]">
                  {t('admin.federationRegistry', 'Maharashtra Cooperative Federation Registry')}
                </span>
              </div>
            </div>
            ...
            <div className="sm:col-span-2 space-y-2 text-xs">
              <div className="p-3 bg-white/80 rounded-xl border border-[#E8E2D5] space-y-1.5">
                <div className="flex justify-between">
                  <span className="text-[#77736B]">{t('admin.docCategory', 'Document Category:')}</span>
                  <strong className="text-[#292824] capitalize">{document.documentType} Verification</strong>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#77736B]">{t('admin.attachedRecord', 'Attached Record:')}</span>
                  <strong className="text-[#292824] truncate max-w-[200px]">{document.title}</strong>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#77736B]">{t('admin.societyStation', 'Society Station:')}</span>
                  <strong className="text-[#80432E]">{worker.societyName || 'Green Residency'}</strong>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#77736B]">{t('admin.verificationChecksum', 'Verification Checksum:')}</span>
              ...
              {document.reviewNotes && (
                <div className="p-2.5 bg-[#FAF7F2] rounded-lg border border-[#E8E2D5] text-[11px] text-[#524E47]">
                  <strong>{t('admin.priorReviewNote', 'Prior Review Note:')}</strong> {document.reviewNotes}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="pt-2 border-t border-[#E8E2D5]/70 flex items-center justify-between text-[10px] text-[#77736B]">
            <span>{t('admin.encryptedStorage', 'Encrypted local custody storage')}</span>
            <span className="text-[#445D3E] font-semibold flex items-center gap-1">
              <Lock className="w-3 h-3" />
              <span>{t('admin.tamperEvident', 'Tamper-evident verification')}</span>
            </span>
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-bold text-[#524E47] block">
            {t('admin.reviewerRemarks', 'Reviewer Remarks / Feedback (Optional)')}
          </label>
          <input
            type="text"
            placeholder={t('admin.remarksPlaceholder', "Add note for worker (e.g. 'Clear copy verified', 'Address matches society ward', etc.)")}
            value={notes}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-2.5 pt-3 border-t border-[#E8E2D5]">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
            ...
            >
              {t('admin.requestCorrection', 'Request Correction')}
            </button>
            <button
              type="button"
              disabled={isProcessing}
            ...
            >
              ...
              <span>{t('admin.approveDoc', 'Approve Document')}</span>
            </button>
          </div>
        </div>
      </div>

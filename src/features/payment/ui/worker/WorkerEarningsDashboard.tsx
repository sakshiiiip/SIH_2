  ...
  const load = useCallback(
    async (silent = false) => {
      ...
      try {
        ...
        setError(t('worker.earningsDashboard.loadError', 'Unable to load earnings data. Please try again.'));
      } finally {
        setLoading(false);
        setRefreshing(false);
      }
  ...
  if (error || !data) {
    return (
      <div className="flex flex-col items-center justify-center py-16 gap-3 text-center">
        ...
        <p className="text-sm text-[#80432E] font-medium">{error ?? t('worker.earningsDashboard.noData', 'No data available.')}</p>
        <button
          type="button"
          onClick={() => load()}
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#2A3927] via-[#364A32] to-[#445D3E] p-6 shadow-float">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-bold uppercase tracking-widest text-[#A8B9A3]">
                {t('worker.earningsDashboard.todayEarnings', "Today's Earnings")}
              </span>
              {todaySnapshot.jobsCompleted > 0 && (
                <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-[#6E8B67]/30 text-[#CFDDD0] border border-[#6E8B67]/40">
                  {todaySnapshot.jobsCompleted} {t('worker.earningsDashboard.jobs', 'jobs')}
                </span>
              )}
            </div>
            ...
            <div className="flex items-end gap-2">
              ...
              {todaySnapshot.totalDeductions > 0 && (
                <span className="text-xs text-[#A8B9A3] mb-1 font-mono">
                  ({t('worker.earningsDashboard.gross', 'gross')} ₹{fmt(todaySnapshot.totalGross)})
                </span>
              )}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 text-[#A8B9A3] text-xs">
              <span className="flex items-center gap-1">
                ...
                {t('worker.earningsDashboard.week', 'Week')}: <strong className="text-white font-mono ml-1">₹{fmt(summary.thisWeekNet)}</strong>
              </span>
              <span className="w-px h-3.5 bg-[#6E8B67]/50" />
              <span className="flex items-center gap-1">
                {t('worker.earningsDashboard.month', 'Month')}: <strong className="text-white font-mono ml-1">₹{fmt(summary.thisMonthNet)}</strong>
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-col items-end gap-3 shrink-0">
            <button
              ...
              title={t('worker.earningsDashboard.refreshEarnings', 'Refresh earnings')}
            >
              <RefreshCw
            ...
            <div className="flex items-center gap-1 px-2.5 py-1.5 bg-white/10 rounded-xl">
              ...
              <span className="text-[#A8B9A3] text-[11px]">{t('worker.earningsDashboard.avgRating', 'avg rating')}</span>
            </div>
          </div>
        </div>
        ...
        <div className="relative z-10 mt-5 pt-4 border-t border-white/10 flex flex-wrap items-center gap-x-6 gap-y-2">
          <div>
            <span className="text-[11px] text-[#A8B9A3] block">{t('worker.earningsDashboard.allTimeNet', 'All-time Net')}</span>
            <span className="text-base font-bold font-mono text-white">
              ₹{fmt(summary.allTimeNet)}
            </span>
          </div>
          <div>
            <span className="text-[11px] text-[#A8B9A3] block">{t('worker.earningsDashboard.totalJobs', 'Total Jobs')}</span>
            <span className="text-base font-bold font-mono text-white">
              {summary.totalJobsCompleted}
            </span>
          </div>
          <div>
            <span className="text-[11px] text-[#A8B9A3] block">{t('worker.earningsDashboard.avgPerJob', 'Avg / Job')}</span>
            <span className="text-base font-bold font-mono text-white">
          ...
          {onViewBreakdown && (
            ...
            >
              {t('worker.earningsDashboard.analytics', 'Analytics')} <ChevronRight className="w-3.5 h-3.5" />
            </button>
          )}
        </div>
      </div>
        ...
        >
          <span className="flex items-center gap-1.5">
            ...
            <span className="text-[11px] font-semibold text-[#77736B] uppercase tracking-wider">
              {t('worker.earningsDashboard.pending', 'Pending')}
            </span>
          </span>
          ...
          <p className="text-[11px] text-[#9A958B]">
            {summary.pendingPaymentCount} {t('worker.earningsDashboard.payments', 'payments')}
          </p>
        </div>
        ...
        <div className="rounded-2xl p-4 bg-[#EFEBF4] border border-[#DFD8E8] shadow-subtle space-y-1">
          <span className="flex items-center gap-1.5">
            ...
            <span className="text-[11px] font-semibold text-[#504161] uppercase tracking-wider">
              {t('worker.earningsDashboard.onHold', 'On Hold')}
            </span>
          </span>
          ...
          <p className="text-[11px] text-[#7A6A8E]">
            {onHold.length} {t('worker.earningsDashboard.disputed', 'disputed')}
          </p>
        </div>
        ...
        <div className="rounded-2xl p-4 bg-[#E6ECE4] border border-[#CFDDD0] shadow-subtle space-y-1">
          <span className="flex items-center gap-1.5">
            ...
            <span className="text-[11px] font-semibold text-[#445D3E] uppercase tracking-wider">
              {t('worker.earningsDashboard.credited', 'Credited')}
            </span>
          </span>
          ...
          <p className="text-[11px] text-[#587352]">{recentCompleted.length} {t('worker.earningsDashboard.recent', 'recent')}</p>
        </div>
      </div>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-xs font-bold uppercase tracking-wider text-[#324F66] flex items-center gap-1.5">
              ...
              {t('worker.earningsDashboard.upcomingPayouts', 'Upcoming Payouts')}
            </h2>
            {onViewHistory && (
              <button
                type="button"
                ...
                className="text-xs font-bold text-[#537895] hover:underline cursor-pointer"
              >
                {t('worker.earningsDashboard.viewAll', 'View all →')}
              </button>
            )}
          </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-[#77736B] mt-0.5">
                    {t('worker.earningsDashboard.release', 'Release')}: <span className="font-mono font-medium text-[#292824]">{fmtDay(p.expectedDisbursementDate)}</span>
                  </p>
                </div>
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-xs font-bold uppercase tracking-wider text-[#364A32] flex items-center gap-1.5">
            ...
            {t('worker.earningsDashboard.todayActivity', "Today's Activity")}
          </h2>
          {onViewHistory && (
            <button
              type="button"
              ...
              className="text-xs font-bold text-[#537895] hover:underline cursor-pointer"
            >
              {t('worker.earningsDashboard.fullHistory', 'Full history →')}
            </button>
          )}
        </div>
        ...
        {recentTxns.length === 0 ? (
          <div className="py-10 rounded-2xl border border-[#E8E2D5] bg-[#FCF9F3] text-center space-y-2">
            ...
            <p className="text-sm font-medium text-[#292824]">{t('worker.earningsDashboard.noActivityToday', 'No activity today')}</p>
            <p className="text-xs text-[#9A958B]">{t('worker.earningsDashboard.completedJobsHint', 'Completed jobs will appear here once processed.')}</p>
          </div>
        ) : (
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-xs font-bold uppercase tracking-wider text-[#364A32] flex items-center gap-1.5">
              ...
              {t('worker.earningsDashboard.recentlyCredited', 'Recently Credited')}
            </h2>
            {onViewHistory && (
              <button
                type="button"
                ...
                className="text-xs font-bold text-[#537895] hover:underline cursor-pointer"
              >
                {t('worker.earningsDashboard.allPayments', 'All payments →')}
              </button>
            )}
          </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-[#77736B] font-mono mt-0.5">
                    {t('worker.earningsDashboard.via', 'via')} {p.disbursementMethod.replace('_', ' ')} · {p.disbursementReference}
                  </p>
                </div>
